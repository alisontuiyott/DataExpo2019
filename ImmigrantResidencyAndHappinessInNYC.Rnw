...
\author{Alison Tuiyott\thanks{Miami University Department of Statistics,  501 E High St, Oxford, OH 45056}\and  Thomas Fisher\samethanks\and  Karsten Maurer\samethanks \and Robert Garrett\samethanks \and Ben Schweitzer\samethanks \and Lydia Carter\samethanks }
\begin{document}
\maketitle
...
\begin{abstract}
Exploring the quality of life of immigrants in New York City through housing and neighborhood conditions by creating a happiness metric. Utilizing data provided by the New York City Housing and Vacancy Survey (NYCHVS), the New York City Police Department, the New York City Department of Education, and the New York City Department of Health and Mental Hygiene and reports from Happy City and the New Economics Foundation, a happy score was assigned to each sub-borough in New York City. This happy score evaluated five main domains: work, place, community, education, and health.


\begin{keywords}
New York City Housing Vacancy Survey, Happy Score, Immigrant Residency, Data Expo
\end{keywords}
\end{abstract}
...
\subsubsection{Place: Rent}

In the NYCHVS data, there is a field that identifies monthly contract rent. We assume this is a relatively decent measure of place. Using this field, we adjust for inflation and calculate the average monthly contract rent per sub-borough. \textbf{Figure 3} is a map of Manhattan illustrating average monthly contract rent by sub-borough.
...
\subsubsection{Education: High School Achievement Rates}
...
\subsubsection{Health: Mortality Age Range}
...
The data ranges from 1999 to 2014. Since the NYCHVS data is collected every three years, we only selected years that overlapped with the NYCHVS data. \textbf{Figure 6} is a map of Manhattan illustrating median death age range by community district.
...
#grid.arrange(fig1,fig2,widths=c(6,6),nrow=3,ncol=2)
ggarrange(fig1,fig2,fig3,fig4,fig5,fig6,widths=c(3,3),heights=c(2,2,2),nrow=3,ncol=2)
@
...
\subsubsection{Aggregating the Data}
...
\textbf{Figure 7} overlays all the different geographic regions that are necessary to calculate the happy score index. Each region has a unique combination of sub-borough, precinct, school district and community district.
...
\subsubsection{Calculating Happy Score Index}
...
\textbf{Figure 8} shows the happy score index  and the immigrant residency for each sub-borough.
...
\textbf{Figure 9} is another visualization to better understand the association between immigrant residency and happy scores.
...
ggplot(data=b2) +
   geom_point(aes(x=immigrantPct,y=happiness,color=name,size=totalNum),alpha=0.7)+
   labs(x="Immigrant Residency", y="Happy Score Index",
        title = "Immigrant Residency and Happy Score Index", caption= "Figure 9") +
   theme_minimal()+
   theme(legend.background = element_rect(colour = "gray20"),
         legend.position = "top",
...
In this analysis, we utilized the NYCHVS data and other external resources to create a happiness metric to evaluate the quality of life for immigrant households in New York City. We took advantage of the happiness score index created by Happy City and the New Economics Foundation to create a metric for each sub-borough in New York City. From the happy score index map visualization, it is clear that most of Manhattan and Staten Island tend to be happier than some areas of Brooklyn/Queens and Bronx. It would be interesting to see if over time, these happy scores change within a given sub-borough. When analyzing the scatter plot visualization with all of the sub-boroughs, it would be interesting to figure out exactly which sub-boroughs were different compared to the borough averages. Overall, we know that happiness, in our calculations, is heavily skewed in favor of wealth. However, we know that being wealthier may not equate to being happier, but more wealth can alleviate other real life problems. 

\begin{references}
\itemsep=0pt
{\footnotesize
\item 
"Happy City Index." What Works Centre for Wellbeing, 27 Oct. 2014, whatworkswellbeing.org/case-study/happy-city-index/.

\item
Lewis, Sam Wren, and Saamah Abdallah. Happy City Index 2016 Report. Happy City Measurement and Policy Team, 2016, Happy City Index 2016 Report, www.happycity.org.uk/wp-content/uploads/2016/10/Happy-City-Index-2016-Report-FINAL.pdf.
