...
The shape file data is provided by the New York City Department of City Planning. This website provides the shape file data for the Census blocks which is what was used in the NYCHVS data, school districts and community districts. The precinct shape files are provided by New York City OpenData.

Starting with the NYCHVS data, we use the pumas function in the tigris package in R to extract New York specific data. We use the geoid's provided by the NYCHVS data to extract the necessary geoid's from the Public Use Microdata Area (PUMA) data. From there we can create an outline of New York City and the small sub-boroughs. The data is read in and projected to the Long Island State Plane coordinate reference system.
...
We intersect all of the geographic regions to create one map with small pieces, each representing a unique combination of the five measures (Income, Rent, Crime, etc.). We calculate a weighted combination of the individual measures within the sub-borough, scaled each happiness measure from 0 (Worst) to 1 (Best), and then took a sum of all five scaled measures to create the Happy Score Index.
